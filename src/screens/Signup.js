...
export default function Signup() {
  const [credentials, setCredentials] = useState({ name: "", email: "", password: "", geolocation: "" ,cords:0})
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("http://localhost:5000/api/auth/createuser", {
      ...
      body: JSON.stringify({ name: credentials.name, email: credentials.email, password: credentials.password, 
        location: credentials.geolocation, cords: credentials.cords })
    ...
    if (json.success) {
      ...
      localStorage.setItem("Xcord",global.lat)
      localStorage.setItem("Ycord",global.long)
      localStorage.setItem('userEmail', credentials.email)
      navigate("/login")
      // global.Xcord=global.lat;
      // global.Ycord=global.long;
